...
import {
  LayoutDashboard, Users, Building2, Briefcase, FileText, Rss, FolderTree, Tag,
  BarChart3, Shield, ClipboardList, Bell, Settings, ChevronLeft, ChevronRight, Brain
} from 'lucide-react';
...
function AescionSymbol({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 300 260" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
      {/* Inverted triangle divided into 4 sections */}
      {/* Top-left: Blue */}
...
const NAV_ITEMS = [
  ...
  { label: 'Company Admins', path: '/admin/company-admins', icon: Users },
  { divider: 'COMPANIES' },
  ...
  { divider: 'JOB FEEDS' },
  { label: 'Feed Sources', path: '/admin/job-feeds', icon: Rss },
  { label: 'Sync History', path: '/admin/job-feeds/sync-history', icon: ClipboardList },
  { label: 'Failed Jobs', path: '/admin/job-feeds/failed-jobs', icon: Shield },
  { divider: 'MANAGEMENT' },
  { label: 'Categories', path: '/admin/categories', icon: FolderTree },
  { label: 'Skills', path: '/admin/skills', icon: Tag },
  { divider: 'ANALYTICS' },
  { label: 'Reports', path: '/admin/reports', icon: Shield },
  ...
  { label: 'ATS Settings', path: '/admin/ats-settings', icon: Brain },
...
export default function AdminSidebar({ isOpen, onToggle }: SidebarProps) {
  ...
  return (
    <aside className={`fixed top-0 left-0 h-full bg-[#0f172a] text-white z-40 transition-all duration-300 flex flex-col ${isOpen ? 'w-64' : 'w-20'}`}>
      <div className="flex items-center justify-between px-3 h-16 border-b border-white/10">
        {isOpen && (
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-[9px] font-bold tracking-widest uppercase text-slate-300 mt-0.5">SUPER ADMIN</span>
            </div>
          </div>
        )}
        {!isOpen && (
        ...
        <button onClick={onToggle} className="p-1.5 rounded-lg hover:bg-white/10 text-[#94a3b8] hover:text-white transition-colors shrink-0">
          ...
          if ('divider' in item) {
            return isOpen ? (
              <div key={i} className="pt-4 pb-1.5 px-2">
                <span className="text-[10px] font-bold text-[#64748b] tracking-widest uppercase">{item.divider}</span>
              </div>
