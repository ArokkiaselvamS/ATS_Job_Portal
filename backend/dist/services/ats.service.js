...
async function calculateATSScore(profileId) {
    ...
    if (profile.professionalSummary && profile.professionalSummary.length > 50) {
        ...
        if (profile.professionalSummary.length > 150)
            scores.keywordMatch = 95;
        else if (profile.professionalSummary.length > 100)
            scores.keywordMatch = 90;
    }
    ...
    if (profile.candidateType === 'EXPERIENCED') {
        ...
        const detailedExp = experiences.filter(e => e.responsibilities && e.responsibilities.length > 50 &&
    ...
    if (education.length >= 2) {
        ...
        if (edu.cgpaPercentage)
            scores.educationMatch = 85;
        if (edu.degree && edu.collegeUniversity)
            scores.educationMatch = 90;
    }
    else {
    ...
    if (profile.targetJobTitles && profile.targetJobTitles.length > 0) {
        ...
        if (profile.professionalHeadline) {
            ...
            const matchesTarget = profile.targetJobTitles.some(title => headlineLower.includes(title.toLowerCase().split(' ')[0]));
            if (matchesTarget)
                scores.jobTitleMatch = 95;
        }
    }
    else {
    ...
    const overall = Math.round(scores.keywordMatch * weights.keywordMatch +
    ...
    if (!profile.githubUrl && skills.some(s => ['TECHNICAL', 'PROGRAMMING_LANGUAGE', 'FRAMEWORK'].includes(s.category))) {
...
async function calculateProfileCompletion(profileId) {
    const profile = await prisma_1.default.profile.findUnique({
        where: { id: profileId },
        include: {
            education: true,
