...
function getExperienceLevelKey(level) {
    if (!level)
        return '';
    return normalizeString(level);
}
function getHighestEducation(edu) {
    let highest = 0;
    for (const e of edu) {
        if (!e.degree)
            continue;
...
function getEducationLevelFromJob(jobRequirements) {
    if (!jobRequirements)
        return 0;
...
function matchesLocation(profileLocation, profileCity, profileCountry, profilePreferredLocations, jobLocation, jobCity, jobCountry) {
    if (!jobLocation && !jobCity && !jobCountry)
        return true;
    ...
    if (normJobCity && normProfileCity === normJobCity)
        return true;
    if (normJobLoc && normProfileLoc.includes(normJobLoc))
        return true;
    if (normJobCountry && normProfileCountry === normJobCountry)
        return true;
    for (const preferred of profilePreferredLocations) {
        const normPreferred = normalizeString(preferred);
        if (normJobCity && normPreferred.includes(normJobCity))
            return true;
        if (normJobLoc && normPreferred.includes(normJobLoc))
            return true;
        if (normJobCountry && normPreferred.includes(normJobCountry))
            return true;
    }
    return false;
}
// ─── MATCH SCORE CALCULATION ─────────────────────────────
function calculateMatchForJob(profile, job) {
    const reasons = [];
    // ── Skills (40%) ──────────────────────────────────────
    ...
    if (jobSkillNames.length > 0) {
        for (const jobSkill of jobSkillNames) {
            const matched = profileSkillNames.some(ps => ps === jobSkill ||
        ...
        for (const missing of [...missingSkills]) {
            const matched = allTechs.some(tech => tech === missing ||
    ...
    // ── Location (5%) ─────────────────────────────────────
    const locationMatch = matchesLocation(profile.location, profile.city, profile.country, profile.preferredLocations || [], job.location, job.city, job.country);
    ...
    // ── Overall ───────────────────────────────────────────
    const overall = Math.round(skillsScore * WEIGHTS.skills +
...
async function getMatchesForUser(userId) {
    ...
    const activeJobs = await prisma_1.default.job.findMany({
        where: {
            status: { in: [client_1.JobStatus.ACTIVE, client_1.JobStatus.PUBLISHED] },
        },
...
async function getMatchForJob(userId, jobId) {
    // Load user profile
    const profile = await prisma_1.default.profile.findUnique({
        where: { userId },
        include: {
    ...
    if (job.status !== client_1.JobStatus.ACTIVE && job.status !== client_1.JobStatus.PUBLISHED) {
        return null;
    }
    const { breakdown, matchingSkills, missingSkills, reasons } = calculateMatchForJob({
        skills: profile.skills,
