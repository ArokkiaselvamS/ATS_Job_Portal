...
function parseResumeText(text) {
    ...
    for (const line of lines) {
        ...
        const matchedHeader = sectionHeaders.find(h => lowerLine === h || lowerLine.startsWith(h + ':') || lowerLine.startsWith(h + 's:'));
        if (matchedHeader) {
            if (currentSection)
...
function parseExperienceSection(text, result) {
...
function parseEducationSection(text, result) {
    const lines = text.split('\n').filter(l => l.trim());
    for (const line of lines) {
...
function parseSkillsSection(text, result) {
...
function parseProjectsSection(text, result) {
    const blocks = text.split(/\n(?=[A-Z])/);
    for (const block of blocks) {
        const lines = block.split('\n').filter(l => l.trim());
        if (lines.length === 0)
...
function parseCertificationsSection(text, result) {
    const lines = text.split('\n').filter(l => l.trim());
    for (const line of lines) {
...
function parseLanguagesSection(text, result) {
    const lines = text.split('\n').filter(l => l.trim());
    for (const line of lines) {
